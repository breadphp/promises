...
class When {
  ...
  public static function resolve($promiseOrValue = null) {
    return static::promiseFor($promiseOrValue);
  }

  public static function reject($promiseOrValue = null) {
    return static::rejectedPromiseFor($promiseOrValue);
  }
  ...
  public static function all($promisesOrValues, $fulfilledHandler = null,
    $errorHandler = null, $progressHandler = null) {
  ...
  public static function any($promisesOrValues, $fulfilledHandler = null,
    $errorHandler = null, $progressHandler = null) {
  ...
  public static function some($promisesOrValues, $howMany,
    $fulfilledHandler = null, $errorHandler = null, $progressHandler = null) {
    return When::resolve($promisesOrValues)->then(function ($array) use (
      $howMany, $fulfilledHandler, $errorHandler, $progressHandler) {
      if (!is_array($array)) {
        $array = array();
      }
      $len = count($array);
      ...
      if (!$toResolve) {
        ...
        $rejectOne = function ($reason, $i) use (&$reasons, &$toReject,
          $deferred) {
          $reasons[$i] = $reason;
          if (0 === --$toReject) {
  ...
  public static function map($promisesOrValues, $mapFunc) {
    return When::resolve($promisesOrValues)->then(function ($array) use (
      ...
      if (!$toResolve) {
        ...
        $resolve = function ($item, $i) use ($mapFunc, &$results, &$toResolve,
          $deferred) {
          When::resolve($item)->then($mapFunc)->then(function ($mapped) use (
            &$results, $i, &$toResolve, $deferred) {
            $results[$i] = $mapped;
            if (0 === --$toResolve) {
              $deferred->resolve($results);
            }
  ...
  public static function reduce($promisesOrValues, $reduceFunc,
    $initialValue = null) {
    return When::resolve($promisesOrValues)->then(function ($array) use (
      $reduceFunc, $initialValue) {
      if (!is_array($array)) {
        $array = array();
      }
      $total = count($array);
      ...
      $wrappedReduceFunc = function ($current, $val) use ($reduceFunc, $total,
        &$i) {
        return When::resolve($current)->then(function ($c) use ($reduceFunc,
          $total, &$i, $val) {
          return When::resolve($val)->then(function ($value) use ($reduceFunc,
            $total, &$i, $c) {
